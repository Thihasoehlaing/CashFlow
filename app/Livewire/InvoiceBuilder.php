...
class InvoiceBuilder extends Component
{
    ...
    {
        $this->currency = old('currency', $invoice?->currency ?? 'MYR');
        $this->discountType = old('discount_type', $invoice?->discount_type);
        $this->discountValue = (float) old('discount_value', $invoice?->discount_value ?? 0);
        $this->taxRate = (float) old('tax_rate', $invoice?->tax_rate ?? Setting::get('default_tax_rate', 8));
        $oldItems = old('items');
    ...
    public function recalculate(): void
    {
        ...
        $totals = app(InvoiceService::class)->calculateTotals($this->items, $this->discountType, $this->discountValue, $this->taxRate);
