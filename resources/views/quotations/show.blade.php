<x-layouts::app :title="$quotation->quotation_number">
    <div class="space-y-5">
        ...
        <section class="card overflow-x-auto">
            <table class="w-full min-w-[760px] text-sm">
                <thead class="text-left text-[var(--color-text-muted)]">
                    <tr><th>#</th><th>{{ __('quotations.description') }}</th><th>{{ __('quotations.type') }}</th><th>{{ __('quotations.qty') }}</th><th>{{ __('quotations.unit_price') }}</th><th>{{ __('quotations.amount') }}</th></tr>
                </thead>
                <tbody>
                    @foreach ($quotation->items as $item)
                        <tr class="border-t border-[var(--color-border)]"><td class="py-3">{{ $loop->iteration }}</td><td>{{ $item->description }}</td><td>{{ $item->item_type }}</td><td class="font-mono">{{ $item->quantity }}</td><td><x-currency-badge :amount="$item->unit_price" :currency="$quotation->currency" /></td><td><x-currency-badge :amount="$item->amount" :currency="$quotation->currency" /></td></tr>
                    @endforeach
                </tbody>
            </table>
        </section>
        ...
        <section class="card flex flex-wrap gap-3">
            <form method="POST" action="{{ route('quotations.status', $quotation) }}">
                @csrf
                @method('PATCH')
                <select class="field" name="status">
                    <option value="accepted">{{ __('common.status.accepted') }}</option>
                    <option value="rejected">{{ __('common.status.rejected') }}</option>
                    <option value="sent">{{ __('common.status.sent') }}</option>
                </select>
                <button class="btn-secondary mt-2">{{ __('quotations.update_status') }}</button>
            </form>
            @if($quotation->status === 'accepted')
                <form method="POST" action="{{ route('quotations.convert', $quotation) }}">
                    @csrf
                    <button class="btn-primary">{{ __('quotations.convert') }}</button>
                </form>
            @endif
        </section>
    </div>
